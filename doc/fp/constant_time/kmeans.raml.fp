...
let rec zip l1 l2 =
  ...
  | Cons(x,xs) ->
    match l2 with
    | Nil -> let unused = consume xs in Nil
...
(*   ifz m *)
(*     (fun unused -> consume ldb; Nil) *)
(*     (fun(m') -> *)
(*        let yes, no = split (fun(x) -> let(pt, i) = x in i = to_int m') ldb in *)
(*        consume(yes,no,m'); *)
...
let initmeans x =
  ...
  let unused = [point1; point2 in point3]
;;
;;
